<script>
  import { contactModalOpen } from '$lib/stores.js'; // 1. Import store

  function openModal() {
    $contactModalOpen = true; // 2. Set store value
  }
</script>
<section
	class="dark:bg-gray-100 dark:text-gray-800 bg-cover bg-center relative"
	style="background-image: url('./phil-5i0GnoTTjSE-unsplash.jpg')"
>
	<div class="absolute inset-0 bg-black opacity-30"></div>

	<div
		class="relative z-10 container mx-auto flex flex-col items-center
					 px-4 py-16 text-center md:py-32 md:px-10 lg:px-32 xl:max-w-3xl"
	>
		<h1 class="text-4xl font-bold leading-none sm:text-5xl">
			Buka Kekuatan
			<span class="dark:text-orange-600">Transformasi Digital Anda</span>
		</h1>
		<p class="px-8 mt-8 mb-12 text-lg">
			Rampingkan Operasi Bisnis Anda dan Tetap Terdepan
		</p>
		<div class="flex flex-wrap justify-center">
			<button
				class="px-8 py-3 m-2 text-lg font-semibold rounded
					 dark:bg-orange-600 dark:text-gray-50
					 hover:dark:bg-orange-700 transition-all duration-300
					 active:scale-95"
				on:click={openModal}
			>
				Mulai Sekarang!
			</button>
		</div>
	</div>
</section>
